...
@if(isset($options->relationship))
    ...
    @if( method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        @if(isset($dataTypeContent->{$row->field}) && !is_null(old($inputKey, $dataTypeContent->{$row->field})))
            <?php $selected_value = old($inputKey, $dataTypeContent->{$row->field}); ?>
        @else
            <?php $selected_value = old($inputKey); ?>
        @endif
    ...
    @endif
@else
    <?php $selected_value = (isset($dataTypeContent->{$row->field}) && !is_null(old($inputKey, $dataTypeContent->{$row->field}))) ? old($inputKey, $dataTypeContent->{$row->field}) : old($inputKey); ?>
    <select class="form-control select2" name="{{ $name }}">
        <?php $default = (isset($options->default) && !isset($dataTypeContent->{$row->field})) ? $options->default : null; ?>
        @if(isset($options->options))
            @foreach($options->options as $key => $option)
